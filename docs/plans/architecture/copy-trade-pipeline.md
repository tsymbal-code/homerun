...
| 17a | Projection-sweep grace guard | `backend/services/signal_bus.py` (`_EXPIRE_SOURCE_GRACE_SECONDS`, `expire_source_signals_except`) | After Plan 0052: `expire_source_signals_except` skips rows younger than 60 s by default. Closes the race where `intent_runtime._project_status_batch`'s snapshot was taken just before a fresh `INSERT` and force-expired the new row 2-3 s after creation. Caller (`intent_runtime.py:3124`) keeps default; pass `min_signal_age_seconds=0.0` to opt out. |
