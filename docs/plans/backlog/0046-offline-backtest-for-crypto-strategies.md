...
> (a) operator wants to evaluate ≥ 10 configurations of
>     `crypto_5m_midcycle` (or any other `crypto_update`-fed strategy)
>     and the live shadow A/B approach (parallel shadow traders,
>     observed via `trader_events.firehose_evaluation`) has not
>     produced statistically meaningful PnL within 2 weeks, OR
> (b) a new crypto-binary strategy is proposed that needs validation
>     before any live capital (even shadow) is allocated, OR
> (c) operator wants walk-forward / rolling-window analysis of an
>     existing crypto strategy's stability over a > 30-day window.
...
- `ssh polyhome-1 'cd /home/polyhome/homerun && docker compose exec -T backend pytest backend/tests/services/test_chainlink_feed.py backend/tests/services/test_strategy_backtester.py backend/tests/services/test_param_optimizer.py -q'`
...
- [ ] Create Alembic migration adding `crypto_oracle_history` table:
...
- [ ] Add the matching SQLAlchemy model `CryptoOracleHistory` to
      [`backend/models/database.py`](../../../backend/models/database.py) alongside other crypto-related models.
- [ ] In [`chainlink_feed.py`](../../../backend/services/chainlink_feed.py), add an async writer hook
...
- [ ] Reuse existing async DB session pattern from sibling services
...
- [ ] Add a TTL housekeeper task: delete rows older than 14 days,
...
- [ ] Update [architecture/execution-and-fills.md](../architecture/execution-and-fills.md)
...
- [ ] In [`strategy_backtester.py`](../../../backend/services/strategy_backtester.py),
...
- [ ] Reconstruct each 5-minute cycle's market dict from the
...
- [ ] For the `book_depth` / `book_fresh` / `vwap_in_range` gates,
...
- [ ] PnL computation: for each emitted opportunity, look up the
...
- [ ] Extend `TradingParameters` dataclass in
...
- [ ] Add matching `ParameterSpec` rows to `DEFAULT_PARAM_SPECS` with
...
- [ ] Update [`docs/strategies/crypto-5m-midcycle.md`](../../strategies/crypto-5m-midcycle.md)
...
- [ ] Move this file from `backlog/` to `completed/` and update the
