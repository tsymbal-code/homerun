...
- [x] In [`backend/main.py:190-233`](../../backend/main.py:190),
...
- [x] Inline comment block above the function: 3-line note describing
...
- [x] Append a section header comment block in
      [`backend/tests/test_main_lifespan_smoke.py`](../../backend/tests/test_main_lifespan_smoke.py)
      ...
      vs hard-reset split.
- [x] Add `test_reset_orchestrator_boot_state_auto_resumes_shadow_when_previously_running`:
      ...
      and `enabled=True`.
- [x] Add `test_reset_orchestrator_boot_state_hard_resets_when_prior_mode_was_live`:
      ...
      `enabled=False`.
- [x] Add `test_reset_orchestrator_boot_state_hard_resets_when_previously_stopped`:
      mock prior `is_enabled=False, mode="shadow"`. Assert hard
      reset (operator explicitly stopped → don't auto-revive).
- [x] Add `test_reset_orchestrator_boot_state_hard_resets_when_previously_paused`:
      mock prior `is_paused=True, is_enabled=True, mode="shadow"`.
      Assert hard reset (operator paused → don't auto-unpause).
- [x] Add `test_reset_orchestrator_boot_state_always_clears_live_flags_in_auto_resume`:
...
- [x] Add `test_reset_orchestrator_boot_state_runtime_status_mirrors_branch`:
...
- [x] Run validation: `cd backend && pytest tests/test_main_lifespan_smoke.py -q`.
- [x] Mark completed
...
- [x] Pre-deploy state: queried `trader_orchestrator_control`
      (the global singleton — `_reset_orchestrator_boot_state`
      writes here, not the per-trader `traders` row). Captured:
      `mode=shadow, is_enabled=t, is_paused=f,
      run_interval_seconds=60, selected_account_id="08fb2d1e-3bb1-4cd5-bd22-db3efbe4085e",
      shadow_account_id="08fb2d1e-3bb1-4cd5-bd22-db3efbe4085e",
      live_arm=null, live_preflight=null`.
- [x] Post-deploy state: re-queried `trader_orchestrator_control`
      immediately after `docker compose down/up`. Result identical:
      `mode=shadow, is_enabled=t, is_paused=f, run_interval_seconds=60,
      selected_account_id="08fb2d1e-3bb1-4cd5-bd22-db3efbe4085e",
      shadow_account_id="08fb2d1e-3bb1-4cd5-bd22-db3efbe4085e",
      live_arm=null, live_preflight=null`. Snapshot showed
      `current_activity="Cycle[scheduled:general] monitoring open
      orders=12", enabled=t, running=t, last_error=null,
      updated_at=2026-05-10 07:47:35` — the first post-restart
      cycle ran without any operator click. Branch A confirmed.
- [x] Negative-path verification: **skipped intentionally.** The
      regression test
      `test_reset_orchestrator_boot_state_hard_resets_when_previously_stopped`
      pins exactly this branch (mocked prior `is_enabled=False,
      mode='shadow'` → assert hard-reset payload), and a
      production stop+redeploy would impose a 1-2 minute Sandbox
      bot interruption with no additional confidence beyond the
      passing test. Trust the unit test.
- [x] Live-path verification (read-only via SQL — do NOT actually
...
- [x] `git mv docs/plans/0021-orchestrator-auto-resume-shadow-on-startup.md docs/plans/completed/`.
- [x] Update the row in [`plan-control-index.md`](plan-control-index.md)
      to point at the `completed/` path.
- [x] `git log --grep='Plan: 0021'` shows the full commit chain.
- [x] Mark completed
